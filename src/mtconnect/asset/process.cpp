...
namespace mtconnect {
  ...
  namespace asset {
    FactoryPtr ProcessArchetype::getFactory()
    {
      ...
      if (!factory)
      {
        auto activity = make_shared<Factory>(Requirements {
        ...
        auto processStep = make_shared<Factory>(Requirements {{
          {"stepId", true},
          {"optional", ValueType::BOOL, false},
          {"sequence", ValueType::INTEGER, false},
          {"Description", false},
        ...
        processStep->setOrder({"Description", "StartTime", "Duration", "Targets", "ActivityGroups"});
